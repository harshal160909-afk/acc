...
export function SettingsView({ config, entries, onSignOut, onEditProfile }: {
  ...
  return <div className="settings-view">
    ...
    <div className="settings-grid">
      <section className="settings-card glass-panel"><header><span>BUSINESS PROFILE</span><strong>{config.businessName}</strong></header><dl><div><dt>Owner</dt><dd>{config.ownerName}</dd></div><div><dt>Contact email</dt><dd>{config.email || "Not added"}</dd></div><div><dt>Financial year</dt><dd>{config.financialYear}</dd></div><div><dt>Book entries</dt><dd>{entries.length}</dd></div><div><dt>Last workspace update</dt><dd>{config.updatedAt ? formatDisplayDate(new Date(config.updatedAt).toISOString().slice(0, 10)) : "Setup record"}</dd></div></dl><footer><button className="button button-secondary" onClick={() => onEditProfile(entries.length > 0)}>Edit business profile</button><button className="button button-quiet" onClick={onSignOut}>Sign out</button></footer></section>
